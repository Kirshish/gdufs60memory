// ==========================================
// memory.js - 记忆交换计划核心功能
// 仅在 submit.html 和 receive.html 中使用
// ==========================================

// 检查是否在正确的页面
const currentPage = winsdow.location.pathname;
const isSubmitPage = currentPage.includes('submit.html');
const isReceivePage = currentPage.includes('receive.html');

// 如果不在功能页面，不执行任何操作
if (!isSubmitPage && !isReceivePage) {
    console.log('ℹ️ memory.js 仅在 submit.html 和 receive.html 中生效');
    // 不执行后续代码
} else {
    console.log('✅ memory.js 已加载');
    
    // 保存记忆（仅在 submit.html 中使用）
    if (isSubmitPage && typeof saveMemory === 'undefined') {
        window.saveMemory = async function() {
            const input = document.getElementById('memory-input');
            if (!input) {
                console.error('❌ 找不到 memory-input 元素');
                return;
            }
    ...
    // 捞取记忆（仅在 receive.html 中使用）
    if (isReceivePage && typeof fetchMemory === 'undefined') {
        window.fetchMemory = async function() {
    ...
    function displayMemory(content, author, date) {
        ...
        if (!memoryDisplay) {
            console.error('❌ 找不到 memory-display 元素');
            return;
        }
